...
class consistency(perturb_class):
    def __init__(self, data_name, add_neg_ratio=0.2, name="consistency"):
    ...
    def substitute_antonym(self, story, per):
        doc = nlp(story)
        text_word_list, subs = [], []
        for tok in doc:
            p = tok.pos_
            ...
            cand = [c for c in cand if nlp(c)[0].pos_ == p]
            if len(cand) and np.random.random() <= per:
                cand = str(np.random.choice(cand))
                cand_tok = nlp(cand)[0]
                cand = transform(tok, cand_tok)
                text_word_list.append(cand)
...
class commonsense(perturb_class):
    ...
    def original_construct(self, origin_data):
        self.id_dict = {}
    ...
    def construct(self, story):
        tok = nlp(story)
        tok_lemma = [t.lemma_ for t in tok]
        text_word_list, sub = [], []
        for w in tok:
            flag = True
            if (w.lemma_ not in self.sw) and (w.tag_ in self.avail_pos_tag) and (w.lemma_ in self.word2kg) and (
