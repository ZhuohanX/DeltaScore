...
class BaseTemplate:
    ...
    def typos(self, sent, typos):
        return Perturb.add_typos(sent, typos)
    ...
    def remove_negation(self, sent):
        try:
            x = Perturb.remove_negation(self.nlp(sent))
            return x if x != None else sent
        except:
            return sent


    def jumble(self, sent, per):
        tokens = [i.text for i in self.nlp(sent)]
        # print(len(tokens))
        # print(len(tokens))
        jumble_len = int(len(tokens) * per)
        start = random.randint(0, len(tokens)-jumble_len)
        # print(start)
        # print(tokens[start:start+jumble_len])
        shuffled_tokens = tokens[start:start+jumble_len].copy()
        random.shuffle(shuffled_tokens)
        # print(len(tokens[:start] + shuffled_tokens + tokens[start+jumble_len:]))
        return ' '.join(tokens[:start] + shuffled_tokens + tokens[start+jumble_len:])
